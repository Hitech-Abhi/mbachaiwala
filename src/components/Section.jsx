...
const Section = ({h3, text, hasBtn=true, btnText, imgSrc, imgSize="70%", backgroundColor, headingColor, textColor, btnBgColor, btnColor}) => {
  ...
  const imgOptions ={
    initial:{scale: 0.1, opacity:0}, whileInView:{scale: 1, opacity:1},
    transition: {
      delay: 0.3,
    },
  };
  ...
  return (
    <section className='section' style={{backgroundColor,}}>
        <div>
            ...
            {hasBtn && <motion.button style={{color: btnColor, backgroundColor: btnBgColor}} data-cursorpointer={true} {...buttonOptions}>{btnText}</motion.button>}
